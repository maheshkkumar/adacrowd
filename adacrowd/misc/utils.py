...
def logger(
    ...
    if baseline:
        cfg_file = open('./config/baselines.py', "r")
    else:
        cfg_file = open('./config/adacrowd.py', "r")
...
def vis_results(exp_name, epoch, writer, restore, img, pred_map, gt_map):
    ...
    for idx, tensor in enumerate(zip(img.cpu().data, pred_map, gt_map)):
        ...
        pil_output = torch.from_numpy(
            tensor[1] / (tensor[2].max() + 1e-10)).repeat(3, 1, 1)
        pil_label = torch.from_numpy(
            tensor[2] / (tensor[2].max() + 1e-10)).repeat(3, 1, 1)
