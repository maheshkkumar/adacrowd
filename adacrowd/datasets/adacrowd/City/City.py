...
class City(Dataset):
    def __init__(
        ...
        print("Scene folder: {}".format(scene_folder))
        ...
        if scene_folder is None:
            ...
            for fol in folders:
                ...
                combined_imgs = [[gui_imgs, crowd_img]
                                 for crowd_img in crowd_imgs]
                self.data_files += combined_imgs
        else:
            img_fold = os.path.join(data_path, scene_folder, 'images')
            images = [os.path.join(img_fold, img) for img in os.listdir(
                img_fold) if img.endswith('.jpg')]
            np.random.shuffle(images)
            gui_imgs = images[:k_shot]
    ...
    def read_image_and_gt(self, fname):
        ...
        crowd_im = read_img(crowd_img)
        gui_imgs = [read_img(gui_img) for gui_img in gui_imgs]
        gt_path = crowd_img.replace('images', 'gt').replace('.jpg', '.csv')
